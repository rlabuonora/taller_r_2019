...
- Hay cosas que no parecen funciones pero son:
```{r}
1 + 2
`+`(1, 2)
```
...
```{r }
lapply(mpg, is.numeric)
```

---
# `vapply`  y `sapply` 

Simplifican el resultado

```{r }
vapply(mpg, is.numeric, logical(1))
sapply(mpg, is.numeric)
...
- Usar split para crear una lista de los subsets de `mtcars` por `mtcars$cyl`
- Estimar la regresión de `mpg` contra `wt` en cada elemento de la lista
- Extraer la pendiente
- Ponerlo en un vector numérico
